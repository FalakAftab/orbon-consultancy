...
class ShortlistController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        $shortlist = Favorite::query()
            ->with(['university', 'program', 'statusHistory', 'notes'])
    ...
    public function updateStatus(Request $request, Favorite $shortlist): JsonResponse
    {
        $validated = $request->validate([
            'status' => ['required', 'string', 'in:' . implode(',', Favorite::STATUSES)],
        ]);

        abort_unless($shortlist->user_id === $request->user()->id, Response::HTTP_FORBIDDEN);

        $oldStatus = $shortlist->status;
        $shortlist->update(['status' => $validated['status']]);

        ApplicationStatusHistory::create([
            'favorite_id' => $shortlist->id,
            'old_status' => $oldStatus,
            'new_status' => $validated['status'],
            'changed_at' => now(),
        ]);

        $shortlist->load(['university', 'program', 'statusHistory', 'notes']);

        return response()->json([
            'data' => ShortlistResource::make($shortlist),
        ]);
    }

    public function storeNote(Request $request, Favorite $shortlist): JsonResponse
    {
        $validated = $request->validate([
